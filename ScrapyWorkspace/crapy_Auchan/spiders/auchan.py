...
class AuchanSpider(scrapy.Spider):
    ...
    def parse_category(self, response):
        _id_subcategorie_forcing= int(response.meta.get('category_id'))+1000 # pour les categorie n'ayant pas de sub categor
        _id_subcategorie_forcing=str( _id_subcategorie_forcing)    
        category =response.meta['category']
        subcategory =response.meta.get('subcategory',response.meta['category'] )# si pas de subcategorie alors subcategory=nom categorie
        ...
        for product in products:
            ...
            if is_old_price:
                ...
                is_out_of_stock = not product.css('div.miniature-notif.notif-discount.discount-amount.discount-product.prio2').get() is not None
